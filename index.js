...
// In-Memory Session Storage for Rate Limiting & Flow Control
const userSessions = {};

// DYNAMIC PRICING ENGINE WITH TAX AND GATEWAY FEES
// Formula: (Base * 1.18 GST) * 1.025 Gateway Fee
function calculateTotalPayable(basePrice) {
    ...
    const withGST = cleanBase * 1.18; // 18% GST
    const totalPayable = withGST * 1.025; // 2.5% Gateway Fee
...
app.post('/webhook', async (req, res) => {
    res.sendStatus(200); // Instant acknowledgement to Meta
    const body = req.body;
        ...
        try {
            ...
            if (value.messages) {
                ...
                if (msgType === 'text') {
                    ...
                    // =========================================================
                    // ⚙️ PRIORITY STEP 1: CAPTURING REPLIES AFTER WEBSITE REDIRECT
                    // =========================================================
                    if (currentStep === 'awaiting_website_action') {
                        if (userText === '1') {
                            ...
                            if (userLang === 'EN') {
                                ...
                                replyText = `💳 *Excellent Choice!* Your session data is validated.🤝\n\nClick the official checkout link below to complete your **Token Booking ($49)** via Razorpay. This will instantly reserve your delivery slot in *Shahid Creatives* automated production queue:\n\n🔗 *Pay Securely Here:* ${dynamicPaymentLink}\n\n*Project Reference ID:* ${uniqueProjectId}`;
                            } else {
                            ...
                            userSessions[from].step = 'main_menu';
                            let replyText = (userLang === 'EN') ? "👤 Perfect! Shahid will connect with you shortly for a strategy sync call to freeze specifications." : "👤 Perfect! Shahid bhai bohot jald aapke sath strategy call par connect karenge taaki requirements ko finalize kiya ja sake. Get ready to launch! 🚀";
                            return sendWhatsAppMessage(from, replyText);
                        }
                    }

                    // =========================================================
                    // 🛡️ PRIORITY STEP 2: WEBSITE LEAD AUTO-DETECTION (60s THROTTLE)
                    // =========================================================
                    if (rawText.includes("Hi Shahid Creatives!") || rawText.includes("lock in my custom website estimate")) {
                        
                        // 60-second window to completely eliminate double lead clicks
                        if (userSessions[from].lastSubmitedTime && (Date.now() - userSessions[from].lastSubmitedTime < 60000)) {
                            console.log(`[DUPLICATE BLOCKED] Prevented identical webhook trigger for user: ${from}`);
                            return;
                        }
                        ...
                        // Execute centralized unified pricing calculation
                        ...
                        // Notification Layout sent to admin
                        const adminNotification = `🌟 *NEW WEBSITE LEAD ARRIVED!* 🌟\n\n📱 *Client:* +${from}\n👤 *Name:* ${clientName}\n✉️ *Email:* ${clientEmail || 'Not Provided'}\n📝 *Plan:* ${projectScope}\n💰 *Value (Calculated):* ₹${finalCalculatedValue}\n\n🤖 *Status:* Throttle locked & securely logged. Check Admin Panel!`;
                    ...
                    // =========================================================
                    // 3. INBOUND CHAT LEAD CAPTURE FLOW (B2B DIRECT)
                    // =========================================================
                    ...
                    if (currentStep === 'ask_name_email') {
                        ...
                        if (userLang === 'EN') {
                            ...
                            replyText = `Thank you, aapki details receive ho gayi hain! 🤝\n\n🔥 *Launch Discount Applied:* Maine aapke profile ke sath **LAUNCH20** (Flat 20% OFF) active kar diya hai.\n\nAap niche diye gaye path par click karr Razorpay se **₹999 Token Booking** complete kar sakte hain. Isse *Shahid Creatives* mein aapka slot automatic book ho jayega:\n\n🔗 *Direct Pay Gateway Link:* ${selfPayLink}\n\n*Project Reference ID:* ${uniqueProjectId}`;
                        }
                        return sendWhatsAppMessage(from, replyText);
                    }

                    // =========================================================
                    // 4. MAIN MENU NAVIGATION ROUTER
                    // =========================================================
                    let replyText = "";
...
app.listen(PORT, () => console.log(`Server deployed safely on port ${PORT}`));
