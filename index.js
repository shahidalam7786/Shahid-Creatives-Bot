...
// ==========================================
// SECURITY NOTE: It's recommended to use process.env.TELEGRAM_TOKEN
...
bot.onText(/\/start/, (msg) => {
    ...
    const welcomeText = "Welcome to Shahid Creatives! 🚀\n\nMain @shahidcreatives_bot hoon. Aapko kis tarah ki madad chahiye?";
    
    const keyboard = {
        reply_markup: {
            keyboard: [
                [{ text: "🛠 Our Services" }, { text: "📞 Contact Us" }]
            ],
...
// Telegram - Handling Button Clicks
bot.on('message', (msg) => {
    const chatId = msg.chat.id;
    const text = msg.text;

    if (text === "🛠 Our Services" || text === "/services") {
        const servicesText = "Hamari main services yeh hain:\n\n1. 🤖 Telegram & WhatsApp Bot Development\n2. 🎨 Graphic Designing\n3. 📈 Digital Marketing\n4. 🎬 Video Editing\n\nOrder karne ke liye hume directly message karein!";
        bot.sendMessage(chatId, servicesText);
    } 
    else if (text === "📞 Contact Us" || text === "/contact") {
        const contactText = "Aap humse yahan contact kar sakte hain:\n\n📧 Email: contact@shahidcreatives.com\n💬 Telegram: @shahidcreatives_bot\n🌐 Website: www.shahidcreatives.com";
        bot.sendMessage(chatId, contactText);
    }
    // Ignore /start as it's handled above
    else if (text && !text.startsWith("/")) {
        if(text !== "🛠 Our Services" && text !== "📞 Contact Us"){
            bot.sendMessage(chatId, "Kripya niche diye gaye buttons ka use karein ya /start type karein.");
        }
    }
});
...
function getBasePriceByPlan(planScope, isUSD = false) {
    ...
    if (isUSD) {
        // 🚀 AI Automation Plans
        if (text.includes("ai content") || text.includes("seo maintainer")) return "77";
        if (text.includes("b2b") || text.includes("wholesale lead")) return "155";
        if (text.includes("service & appointment") || text.includes("appointment lead")) return "155";
        if (text.includes("e-commerce sales automation") || text.includes("automation retainer")) return "155";
        if (text.includes("complete digital sales") || text.includes("digital sales engine")) return "311";
        ...
        // 🚀 AI Automation Plans
        if (text.includes("ai content") || text.includes("seo maintainer")) return "4999";
        if (text.includes("b2b") || text.includes("wholesale lead")) return "9499";
        if (text.includes("service & appointment") || text.includes("appointment lead")) return "9499";
        if (text.includes("e-commerce sales automation") || text.includes("automation retainer")) return "9499";
        if (text.includes("complete digital sales") || text.includes("digital sales engine")) return "18999";
...
app.post('/send-client-credentials', async (req, res) => {
    try {
        ...
        sendWhatsAppMessage("917529839762", adminAlertText); 
...
// 🟢 ROUTE HANDLER: Payment Reminders
...
app.get('/webhook', (req, res) => {
    const VERIFY_TOKEN = process.env.VERIFY_TOKEN || "shahid_creatives_secret_token_123";
...
app.post('/webhook', async (req, res) => {
    ...
    if (body.object === 'whatsapp_business_account' && body.entry) {
        try {
            ...
            if (value.messages) {
                ...
                if (msgType === 'text') {
                    ...
                    // 🎯 HIGH-PRIORITY INTERCEPTOR STATE: NUDGES
                    ...
                    if (currentStep === 'ask_name_email') {
                        ...
                        // 🎯 ADMIN ALERT COMPLETE PRICE DETAIL
        ...
        } catch (error) { console.error("Webhook processing logic error.", error); }
    }
});
...
async function finalizeConsultationLead(from, textInput, res) {
    ...
    sendWhatsAppMessage("917529839762", comprehensiveAdminAlert); 
...
async function sendWhatsAppMessage(to, text) {
    // SECURITY NOTE: It's recommended to use environment variables for this.
    const SECURED_ACCESS_TOKEN = process.env.WHATSAPP_TOKEN || 'YOUR_WHATSAPP_TOKEN_HERE'; 
    const DEFAULT_PHONE_NUMBER_ID = process.env.PHONE_NUMBER_ID || "1202984902891472"; 
